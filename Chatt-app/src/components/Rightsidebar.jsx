...
const Rightsidebar = ({ selecteduser, setselecteduser }) => {
  return selecteduser && (
    <div className={`bg-[#3A3B52]/90 text-white w-full h-full relative overflow-y-auto scrollbar-hide ${selecteduser ? 'max-md:hidden' : ''}`}>
      {/* Profile Section */}
      <div className='pt-8 pb-4 flex flex-col items-center gap-3 text-sm font-light px-4'>
        ...
        <div className='text-center'>
          ...
          <p className='mt-2 text-gray-300 max-w-xs'>{selecteduser.bio}</p>
        </div>
      </div>
